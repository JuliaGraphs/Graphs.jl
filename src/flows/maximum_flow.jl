...
function Base.getindex(d::DefaultCapacity{T}, s::Integer, t::Integer) where {T}
    if Graphs.has_edge(d.flow_graph, s, t)
        one(T)
    else
        zero(T)
    end
...
Base.size(d::DefaultCapacity) = (Int(d.nv), Int(d.nv))
Base.transpose(d::DefaultCapacity) = DefaultCapacity(reverse(d.flow_graph))
Base.adjoint(d::DefaultCapacity) = DefaultCapacity(reverse(d.flow_graph))
...
function maximum_flow(
    ...
    source::Integer,                       # the source vertex
    target::Integer,                       # the target vertex
    capacity_matrix::AbstractMatrix=DefaultCapacity(flow_graph);
    algorithm::AbstractFlowAlgorithm=PushRelabelAlgorithm(),
    restriction::Real=0,                  # keyword argument for restriction max-flow
)
    if restriction > 0
        return maximum_flow(
            flow_graph, source, target, min.(restriction, capacity_matrix), algorithm
        )
    end
