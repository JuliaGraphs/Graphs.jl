...
function Graphs.outneighbors(g::IGraphs.IGraph, v::Integer)
    ...
    ret = LibIGraph.igraph_neighbors(g.objref, res.objref, v-1, mode, LibIGraph.IGRAPH_NO_LOOPS, LibIGraph.IGRAPH_NO_MULTIPLE)
...
function sir_model(g::Graphs.AbstractGraph, ::IGraphAlgorithm; beta=0.1, gamma=0.1, no_sim=100)
    ig = igraph(g)
...
function community_leiden(g::Graphs.AbstractGraph, ::IGraphAlgorithm; resolution=1.0, beta=0.01, n_iterations=10, kwargs...)
    ig = igraph(g)
...
function layout_kamada_kawai(g::Graphs.AbstractGraph, ::IGraphAlgorithm; maxiter=100, epsilon=0.0, kkconst=0.0, kwargs...)
    ig = igraph(g)
    n = Graphs.nv(g)
...
function Graphs.betweenness_centrality(g::Graphs.AbstractGraph, ::IGraphAlgorithm; weights=nothing, normalized=true, kwargs...)
    ig = igraph(g)
    n = Graphs.nv(g)
...
function Graphs.pagerank(g::Graphs.AbstractGraph{U}, ::IGraphAlgorithm; damping=0.85, weights=nothing, kwargs...) where U<:Integer
    ig = igraph(g)
    n = Graphs.nv(g)
    res = IGraphs.IGVectorFloat(; _uninitialized=Val(true))
    LibIGraph.igraph_vector_init(res.objref, n)
    ...
    ret = LibIGraph.igraph_pagerank(ig.objref, w.objref, res.objref, val, damping, Graphs.is_directed(g), LibIGraph.igraph_vss_all(), LibIGraph.IGRAPH_PAGERANK_ALGO_PRPACK, IGraphs.IGNull().objref)
