...
include("../../src/centrality/edge-betweenness.jl")
...
for g in test_generic_graphs(a2)
    ...
    @test isapprox(
        betweenness_centrality(g, vertices(g), distmx2; normalize=false), [0.0, 1.0, 0.0]
    )
    @test isapprox(
        betweenness_centrality(g, vertices(g), distmx2; normalize=false, endpoints=true),
        [4.0, 5.0, 4.0],
    )
    @test isapprox(
        betweenness_centrality(g, vertices(g), distmx2; normalize=true), [0.0, 0.5, 0.0]
    )
    @test isapprox(
