"""
    function dinic(residual_graph, source, target, capacity_matrix)
...
@traitfn function dinic(
    ...
    while true
        augment = dinic_blocking_flow!(
            residual_graph, source, target, capacity_matrix, flow_matrix, P
        )
        augment == 0 && break
        flow += augment
    end
...
"""
    dinic_blocking_flow!(residual_graph, source, target, capacity_matrix, flow-matrix, P)

Like `blocking_flow`, but requires a preallocated parent vector `P`.
"""
function dinic_blocking_flow! end
@traitfn function dinic_blocking_flow!(
    residual_graph::::IsDirected,     # the input graph
    source::Integer,                     # the source vertex
    target::Integer,                     # the target vertex
    capacity_matrix::AbstractMatrix{T},  # edge flow capacities
...
"""
    dinic_blocking_flow(residual_graph, source, target, capacity_matrix, flow-matrix)
