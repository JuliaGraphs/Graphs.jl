@testset "Dinic" begin
    ...
    for fg in (flow_graph, Graphs.DiGraph{UInt8}(flow_graph), Graphs.DiGraph{Int16}(flow_graph))
      ...
      @test @inferred(Graphs.dinic_impl(residual_graph, 1, 8, Graphs.DefaultCapacity(residual_graph)))[1] == 3
      ...
      function test_blocking_flow(residual_graph, source, target, capacity_matrix, flow_matrix)
          ...
          @test @inferred(Graphs.blocking_flow(h, source, target, capacity_matrix, flow_matrix)) == 0
          ...
          @test @inferred(Graphs.blocking_flow(h, source, target, capacity_matrix_, flow_graph_)) > 0

          #test with connected graph
          @test @inferred(Graphs.blocking_flow(residual_graph, source, target, capacity_matrix, flow_matrix)) > 0
      end
