...
function multiroute_flow(
    ...
    return maximum_flow(
        flow_graph, source, target, capacity_matrix; algorithm=flow_algorithm, restriction=x
    )
end
...
function multiroute_flow(
    ...
    source::Integer,                                 # the source vertex
    target::Integer,                                 # the target vertex
    capacity_matrix::AbstractMatrix{T}=DefaultCapacity(flow_graph);
    flow_algorithm::AbstractFlowAlgorithm=PushRelabelAlgorithm(),
    mrf_algorithm::AbstractMultirouteFlowAlgorithm=KishimotoAlgorithm(),
    ...
    λ = maximum_flow(
        flow_graph, source, target, DefaultCapacity(flow_graph); algorithm=flow_algorithm
    )[1]
    ...
    return multiroute_flow(
        flow_graph, source, target, capacity_matrix, flow_algorithm, mrf_algorithm, routes
    )
end
