@testset "Edmonds Karp" begin
    ...
    for fg in (flow_graph, Graphs.DiGraph{UInt8}(flow_graph), Graphs.DiGraph{Int16}(flow_graph))
        ...
        @test @inferred(Graphs.edmonds_karp_impl(residual_graph, 1, 8, Graphs.DefaultCapacity(residual_graph)))[1] == 3

        # Test with capacity matrix
        @test @inferred(Graphs.edmonds_karp_impl(residual_graph, 1, 8, capacity_matrix))[1] == 28
        ...
        function test_find_path_types(residual_graph, s, t, flow_matrix, capacity_matrix)
            v, P, S, flag = Graphs.fetch_path(residual_graph, s, t, flow_matrix, capacity_matrix)
